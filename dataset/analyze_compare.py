...
INDEX = 0
INPUT_FILE = 'Dataset_120.xlsx'
OUTPUT_LOG = 'compare_output.log'
SUMMARY_FILE = 'compare_summary.xlsx'
...
# ===== Define vulnerability detectors and their matching keywords =====
detectors = {
    "pyth-deprecated-functions": "Pyth deprecated function",
    "pyth-unchecked-confidence": "confidence level",
    "pyth-unchecked-publishtime": "publishTime",
    "chronicle-unchecked-price": "Chronicle price",
    "gelato-unprotected-randomness": "request randomness",
    "chainlink-feed-registry": "Feed Registry"
}
...
def analyze_contract(contract_address):
    ...
    detector_list = ",".join(detectors.keys())
    COMMAND = (
        f"slither --detect {detector_list} "
    ...
    # Check for each detector's matching keyword in the output
    for detector, keyword in detectors.items():
        if keyword in full_output:
            result_dict[detector] = 1
